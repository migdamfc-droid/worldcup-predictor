  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-4xl px-4 py-16 sm:py-24 text-center">
        ...
        <p className="mx-auto mb-8 max-w-lg text-base text-zinc-500 dark:text-slate-400">
        ...
        <div className="mt-20 grid gap-5 sm:grid-cols-3">
          {[
            ...
            <div key={item.title} className="glass-card p-5 text-left">
              <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-lg bg-zinc-200 dark:bg-zinc-800">
                {item.icon}
              </div>
              <h3 className="mb-1.5 text-sm font-semibold">{item.title}</h3>
              <p className="text-xs leading-relaxed text-zinc-500 dark:text-slate-400">{item.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-14 glass-card p-6">
          ...
          <div className="grid gap-px overflow-hidden rounded-lg bg-zinc-200 dark:bg-zinc-900 sm:grid-cols-3">
            {[
              ...
              <div
                key={item.label}
                className="flex items-center justify-between bg-white px-4 py-2.5 dark:bg-zinc-950"
              >
                <span className="text-xs text-zinc-500 dark:text-slate-400">{item.label}</span>
                <span className="text-xs font-semibold text-zinc-900 dark:text-white">+{item.pts}</span>
              </div>
            ))}
          </div>
        </div>
